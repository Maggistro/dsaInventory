...
const buildTable = (inventory) => {
    ...
    const header = 'Itemname'.padEnd(columnSizes[0]) + ' | ' + 'Anzahl'.padEnd(columnSizes[1]) + ' | ' + 'Gewicht'.padEnd(columnSizes[2]) + '\n';

    return `Inventar ${inventory.name}: \n`
        + '```' 
        + inventory.items.reduce(
        (table, item) => 
            table + item.name.padEnd(columnSizes[0]) 
